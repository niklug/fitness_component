 ...
 */
class FitnessModelsessionfocuses extends JModelList {
    ...
    protected function getStoreId($id = '') {
        ...
        $id.= ':' . $this->getState('filter.session_type');
    ...
    protected function getListQuery() {
        ...
        $query->select(
                $this->getState(
                        'list.select', 'a.*,  c.name AS category_name, t.name AS session_type_name'
                )
        );
        $query->from('`#__fitness_session_focus` AS a');
        $query->leftJoin('#__fitness_categories AS c ON c.id = a.category_id');
        $query->leftJoin('#__fitness_session_type AS t ON t.id = a.session_type_id');
    ...
    if (is_numeric($published)) {
       ...
       // Filter by session type
        ...
        if (is_numeric($session_type)) {
            $query->where('t.id = '.(int) $session_type);
        } 
