...
<form action="<?php echo JRoute::_('index.php?option=com_fitness&view=business_profiles'); ?>" method="post" name="adminForm" id="adminForm">
	<fieldset id="filter-bar">
		<div class="filter-search fltlft">
			<label class="filter-search-lbl" for="filter_search">Business Name:</label>
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_FITNESS_BUSINESS_PROFILES_SECONDARY_ADMINISTRATOR', 'a.secondary_administrator', $listDirn, $listOrder); ?>
				</th>
				...
				<th width="10%">
					...
					<?php if ($canOrder && $saveOrder) :?>
						<?php echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'business_profiles.saveorder'); ?>
					<?php endif; ?>
				</th>
                <?php } ?>
		...
		<?php foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php if (isset($item->checked_out) && $item->checked_out) : ?>
					<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'business_profiles.', $canCheckin); ?>
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_fitness&task=business_profile.edit&id='.(int) $item->id); ?>">
				...
				<td>
					<?php echo $item->primary_administrator; ?>
				</td>
				<td>
					<?php echo $item->secondary_administrator; ?>
				</td>


                <?php if (isset($this->items[0]->state)) { ?>
				    <td class="center">
					    <?php echo JHtml::_('jgrid.published', $item->state, $i, 'business_profiles.', $canChange, 'cb'); ?>
				    </td>
                <?php } ?>
                <?php if (isset($this->items[0]->ordering)) { ?>
				    <td class="order">
					    <?php if ($canChange) : ?>
						    <?php if ($saveOrder) :?>
							    <?php if ($listDirn == 'asc') : ?>
								    <span><?php echo $this->pagination->orderUpIcon($i, true, 'business_profiles.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								    <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'business_profiles.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							    <?php elseif ($listDirn == 'desc') : ?>
								    <span><?php echo $this->pagination->orderUpIcon($i, true, 'business_profiles.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								    <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'business_profiles.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							    <?php endif; ?>
						    <?php endif; ?>
